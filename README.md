...
**Prerequisites:** Node.js 22 or newer. `VITE_SUPABASE_ANON_KEY` must be set — in `.env.local` for development or as a Docker build arg for production. `VITE_GPU_API_BASE` is optional (used as a fallback telemetry host).
...
Two-stage Docker build (`Dockerfile`), `node:22-alpine` → `nginx:alpine`:
